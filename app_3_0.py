    ...
    if 'message' in update:
        if 'text' in update['message']:
            ...
            if message == '/start':
                ...
                reply_markup = {'inline_keyboard': [[{'text': "Mahsulotlarimni tahrirlash", 'callback_data': f"ingredients {update['message']['from']['id']}"}], [{'text': "Davom etish", 'callback_data': f"choose {message_id} 2"}]]}
        ...
        if data.split()[0] == 'health':
            ...
            markup = {'inline_keyboard': [[{'text': "Menda qandli diabet mavjud 🤚", 'callback_data': f"diabetes {data.split()[1]}"}], [{'text': "Menda ortiqcha vazn bor 🤚", 'callback_data': f"weight {data.split()[1]}"}], [{'text': "Men ba'zi ovqatlarni xush ko'rmayman 🤚", 'callback_data': f"preference {data.split()[1]}"}], [{'text': "Davom ettirish ⏩", 'callback_data': f"ingredients 0"}]]}
            ...
            callback(chat_id, 0, 50, edit_id, True, int(data.split()[1]))
...
def callback(user_id, start, end, message_id, is_first, update):
    ...
    reply_markup['inline_keyboard'].append([{'text': f"Davom ettirish ⏩", 'callback_data': f"choose {message_id} 1"}])
    if is_first:
